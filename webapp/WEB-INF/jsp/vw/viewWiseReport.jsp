...
<head>
 ...
 <style>
    /* CSS to make font color black for all table elements */
    table, th, td {
        color: black;
    }
</style>
						...
						<div class="col-md-3">
								<div class="form-group">
									<label>Financial Year</label> <select id="financialyear"
										name="financialyear" class="custom-select">
										<option value="">Please Select</option>
										<c:forEach items="${financialyear}" var="financialyear">
											<option value="${financialyear}"
												<c:if test="${financialyear == year}">selected</c:if>>${financialyear}</option>
										</c:forEach>
									</select>
								</div>
						</div>
              ...
              <c:if test="${not empty zoneWiseCounts}">
					<div class="">
						<table id="example1" class="table table-bordered table-striped table-container">
							...
							<tbody>
								<c:forEach items="${zoneWiseCounts}"
									var="zoneWiseCounts">
									<tr>
...
<script>
		function submitForm() {
			document.forms["viewWiseReport"].submit();
		}
	</script>
