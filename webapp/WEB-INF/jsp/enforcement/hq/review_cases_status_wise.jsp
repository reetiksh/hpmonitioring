	...
	<table id="scrutinyListTable" class="table table-bordered table-striped table-responsive">
		<thead>
			<tr>
					...
					<th style="text-align: center; vertical-align: middle;">Assigned on (DD-MM-YYYY)</th>
			</tr>
		</thead>
		<tbody>
		    <c:forEach items="${reviewCasesList}" var="enforcementCase">             
			<tr>
				<td>${enforcementCase.id.GSTIN}</td>
				<td>${enforcementCase.taxpayerName}</td>
				<td>${enforcementCase.caseLocation.locationName}</td>
				<td>${enforcementCase.category.name}</td>
				<td>${enforcementCase.id.period}</td>
				<td><fmt:formatDate value="${enforcementCase.id.caseReportingDate}"  pattern="dd-MM-yyyy" /></td>
				<td>${enforcementCase.indicativeTaxValue}</td>
				<td>${enforcementCase.parameter}</td>
				<td><fmt:formatDate value="${enforcementCase.id.caseReportingDate}"  pattern="dd-MM-yyyy" /></td>
			</tr>
			</c:forEach>
		</tbody>
