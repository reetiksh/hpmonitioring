...
<head>
  ...
  <!-- Only fix the collapsed state gap (do NOT set a new base margin) -->
  <style>
    .sidebar-collapse .app-main { margin-left: 0px !important; }
...
<div class="app-wrapper">
  ...
  <!-- Imports (no hard-coded header/sidebar) -->
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped align-middle">
                  ...
                  <c:forEach items="${userList}" var="object">
                    <tr>
                      <td>
                        <c:out value="${object.firstName}"/> <c:out value="${object.middleName}"/> <c:out value="${object.lastName}"/>
                      </td>
                      ...
                      <td class="text-center">
                        <button type="button" title="Edit User" class="btn btn-info"
                                onclick="updateUser('${object.userId}')">
...
<script>
  ...
  // Fallback: ensure the sidebar toggle actually collapses the margin
  document.querySelectorAll('[data-lte-toggle="sidebar"]').forEach(btn => {
    btn.addEventListener('click', e => {
      // If AdminLTE handled it, body will already get classes.
      // This is a safeguard to remove the left gap if it didn't.
      setTimeout(() => {
        const body = document.body;
        // If sidebar element is hidden but margin still reserved, force collapse class.
        const aside = document.querySelector('.app-sidebar, .main-sidebar');
        const asideHidden = aside && (getComputedStyle(aside).display === 'none' || aside.offsetWidth === 0);
        if (asideHidden && !body.classList.contains('sidebar-collapse')) {
  ...
  // Update user action with CSRF + confirm
  function updateUser(userId) {
    ...
    inCsrf.type = 'hidden';
    inCsrf.name = '${_csrf.parameterName}';
    inCsrf.value = '${_csrf.token}';
