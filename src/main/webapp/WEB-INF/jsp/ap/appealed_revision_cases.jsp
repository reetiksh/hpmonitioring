...
<head>
  ...
  <title>Appeal Revision Cases List</title>
...
<div class="wrapper">
  ...
  <jsp:include page="../ap/appeal_revision_reject_pop_up.jsp"/>
  <jsp:include page="../ap/appeal_revision_approve_pop_up.jsp"/>
          ...
          <div class="col-12">
            <div class="card-primary">
              <div class="card-header">
                <h3 class="card-title">Appeal/Revision Cases</h3>
              </div>
            ...
            <div class="alert alert-success" role="alert" style="display: none;" id="appealRevisionCaseRejectedAlertFail">
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    <c:forEach items="${enforcementAppealRevisionCasesList}" var="object" varStatus="loop">
                      <tr>
                        ...
                        <td>
                        <c:forEach items="${object.remark}" var="remark" varStatus="loop">
                          <i class="fas fa-pen nav-icon fa-xs"></i> <c:out value="${remark}" /><br>
                        </c:forEach>
                      </td>
                      
                      <td>
                        <c:forEach items="${object.apRemarks}" var="apRemark" varStatus="loop">
                          <i class="fas fa-pen nav-icon fa-xs"></i> <c:out value="${apRemark}" /><br>
                        </c:forEach>
                      </td>
                        <td style="text-align: center; vertical-align: middle;"><a href="/ap/downloadFile?fileName=${object.appealRevisionFilePath}"><button type="button" onclick="" class="btn btn-primary"><i class="fas fa-download"></i></button></a></td>
                        <td>
                          <button type="button" onclick="callAppealRevisioinApprovedCase('${object.id.GSTIN}','${object.id.caseReportingDate}','${object.id.period}','${object.appealRevisionFilePath}');" class="btn btn-primary" id="appealRevisioinApproveBtn"  style="margin-bottom: 4px">Approve</button>
                          <button type="button" onclick="callAppealRevisioinRejectedCase('${object.id.GSTIN}','${object.id.caseReportingDate}','${object.id.period}','${object.appealRevisionFilePath}');" class="btn btn-primary" id="appealRevisioinRejectBtn">Revert</button>
                        </td>
                      </tr> 
                    </c:forEach>
                  </tbody>
...
<script>
	function callAppealRevisioinApprovedCase(gstin,reportingDate,period,filePath) {
	  $("#appRevApproveGstiNo").val(gstin);
	  $("#appRevApproveReportingdate").val(reportingDate);
	  $("#appRevApprovePeriod").val(period);
	  $("#appealRevisionApproveFilePath").val(filePath); 
  	  $("#appealRevisonApproveModal").modal('show');
	}
	
	function callAppealRevisioinRejectedCase(gstin,reportingDate,period,filePath){
	 $("#appealRevisionRejectGstin").val(gstin);
	  $("#appealRevisionRejectPeriod").val(period);
	  $("#appealRevisionRejectCaseReportingDate").val(reportingDate);
	  $("#appealRevisionRejectFilePath").val(filePath);
	  
	$("#appealRevisonRejectModal").modal('show');
	}
