          ...
          <tr>
            ...
            <td class="text-center">
              <c:if test="${item.uploadedFileName != null}">
                <a href="/fo/downloadUploadedPdfFile?fileName=${item.uploadedFileName}" aria-label="Download case PDF">
                  <button type="button" class="btn btn-primary"><i class="fa fa-download"></i></button>
                </a>
              </c:if>
            </td>
            <td>
              <div><button type="button" class="btn btn-primary viewbtn mb-2" onclick="view('${item.GSTIN_ID}','${item.caseReportingDate_ID}','${item.period_ID}')">Update</button></div>
              ...
              <c:if test="${item.caseIdUpdate eq 'yes'}">
                <div><button type="button" class="btn btn-primary mt-2" onclick="updateCaseId('${item.GSTIN_ID}','${item.caseReportingDate_ID}','${item.period_ID}','${item.caseId}')">Update Case ID</button></div>
              </c:if>
            </td>
          </tr>
        </c:forEach>
      ...
      <form method="POST" name="updatecaseidForm" id="updatecaseidForm" action="update_caseid" enctype="multipart/form-data">
        <div class="modal-body">
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="form-group">
                <label for="caseid">Old Case ID</label>
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="form-group">
                <label for="newcaseid">New Case ID Proposed<span style="color:red;"> *</span></label>
          ...
          <!-- accept attribute fixed to .pdf (no logic change) -->
  ...
  function view(gst, date, period) {
    $.ajax({
      ...
      success: function (result) {
        ...
        if (result == 'true') {
          $("#updateSummaryViewBody").load('/fo/scrutiny_update_pop_up_view/id?gst=' + gst + '&date=' + date + '&period=' + period, function (response, status) {
    ...
    $("#updateSummaryViewBtn").on("click", function () {
      $.ajax({
        ...
        success: function (result) {
          const myJSON = JSON.parse(result);
          if (result == 'true') {
            $("#updateSummaryViewBody").load('//fo/view_case', function (response, status) {
              if (status == 'success') { $("#updateSummaryViewModal").modal('show'); } else { console.log("failed"); }
            });
          } else if (result == 'false') {
            window.location.reload();
        ...
        $.ajax({
          ...
          success: function (result) {
            ...
            if (result == 'true') {
              ...
              $("#loader").show();
              setTimeout(function () {
                $("#dataListDiv").load('/fo/get_scrutiny_data_list?id=' + categoryId + '&parameterId=' + selectedValue, function (response, status) {
                  $("#loader").hide();
