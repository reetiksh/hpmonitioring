			...
			<tbody>
				...
				<tr>
					...
					<td>
						...
						<button type="button" style="margin-bottom:5px;" class="btn btn-primary viewbtn" onclick="AssignCaseToOfficer('${item.GSTIN_ID}' , '${item.caseReportingDate_ID}' , '${item.period_ID}')" >Assign for Update</button>			
							<c:choose>  
...
<!-------------------------------------- Assign Case to Officer pop up start ----------------------------------------->
<div class="modal fade" id="showFoAssignmentModal" tabindex="-1" role="dialog" aria-labelledby="showFoAssignmentModalTitle" aria-hidden="true" >
			<div class="modal-dialog modal-dialog-centered modal-sm" role="document" >
				<div class="modal-content">
					...
					<div class="modal-body" id="showFoCaseAssignmentDetails" >
					</div>
				</div>
			</div>
		</div>
<!-------------------------------------- Assign Case to Officer pop up end ------------------------------------------->
...
function AssignCaseToOfficer(GSTIN, caseReportingDate, period){
	...
	$.ajax({url: '/checkLoginStatus',
		...
		success: function(result){
			const myJSON = JSON.parse(result);
			if(result=='true'){
				$("#showFoCaseAssignmentDetails").load(link, function(response, status, xhr){
					if(status == 'success'){
							$("#showFoAssignmentModal").modal('show');
					}else{
						console.log("failed");
					}
				});
...
<script>
	// This script will run when the parameter drop down will be selected/change the value
	$(function() {
				                 ...
				                 success: function(result){
				                  const myJSON = JSON.parse(result);
				               	  if(result=='true'){
										...
										$("#loader").show();
										setTimeout(function() {
											$("#dataListDiv").load('/fo/update_summary_data_list?id=' + categoryId + '&parameterId=' + selectedValue + extendedUrl,
													function(response, status, xhr) {
