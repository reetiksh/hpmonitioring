...
<div class="modal fade" id="closeCaseModal" tabindex="-1" role="dialog" aria-labelledby="closeCaseModalTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
     <form method="POST" action="fo_close_raised_query_cases" name="closeCaseForm" >
      <div class="modal-header">
...
<div class="modal fade" id="updateCaseidModal" tabindex="-1" role="dialog" aria-labelledby="updateCaseidModal" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <form method="POST" name="updatecaseidForm" id="updatecaseidForm" action="update_raisedquery_caseid" enctype="multipart/form-data" >
      <div class="modal-body">
		  ...
		  <div class="col-md-12">
				<div class="form-group">
					...
					<input class="form-control" id="newcaseid" maxlength="15" pattern="[A-Za-z0-9]{15}"  name="caseid" title="Please enter 15-digits alphanumeric Case Id" required />
				</div>
		   </div>
		   </div>
           ...
           <div class="btn-group btn-group-vertical" role="group" data-toggle="buttons" style="width: 100%;">
            ...
            <div class="form-group" style="margin-bottom: 0rem;">
            <label for="message-text" class="col-form-label">Upload File<span style="color: red;"> *</span></label>
            </div>
	  ...
	  $.ajax({url: '/checkLoginStatus',
          ...
          async: false,
						success: function(result){
								...
								if(result=='true'){
									$("#updateSummaryViewBody").load('/fo/raised_query_view_case/id?gst='+gst+'&date='+date+'&period='+period, function(response, status, xhr){
											if(status == 'success'){
	  ...
	  $("#updateSummaryViewBtn").on("click", function(){
		  $.ajax({url: '/checkLoginStatus',
	          ...
	          success: function(result){
							const myJSON = JSON.parse(result);
							if(result=='true'){
								$("#updateSummaryViewBody").load('//fo/view_case', 
...
<script>
	$(function() {
		$("#parameter").on('change',function() {
			...
			setTimeout(function() {
				$("#dataListDiv").load('${pageContext.request.contextPath}/fo/raised_query_update_summary_data_list?id=' + categoryId + '&parameterId=' + selectedValue,
					function(response, status, xhr) {
						$("#loader").hide();
						if (status == 'success') {
							console.log("success");
