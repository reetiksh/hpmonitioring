	...
	<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
		<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
			<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %>
				...
				<div class="row">
					<c:if test="${!empty parameters}">
						<div class="col-md-6 form-group">
							...
							<select id="parameter" name="parameter" class="selectpicker col-md-8" data-live-search="true"
								title="Please Select Paramater">
								...
								<tr>
									...
									<td>
										<c:if test="${item.uploadedFileName != null}">
											<a href="/fo/downloadUploadedPdfFile?fileName=${item.uploadedFileName}"><button type="button"
													class="btn btn-primary"><i class="fa fa-download"></i></button></a>
										</c:if>
									</td>
									<td>
										...
										<c:if test="${item.caseIdUpdate eq 'yes'}">
											<button type="button" style="margin-bottom:5px;margin-top:5px;" class="btn btn-primary"
												onclick="updateCaseId('${item.GSTIN_ID}' , '${item.caseReportingDate_ID}' , '${item.period_ID}' , '${item.caseId}')">Update
												Case ID</button>
										</c:if>
					...
					<div class="modal-dialog modal-dialog-centered" role="document">
						<div class="modal-content">
							<form method="POST" action="fo_close_rejected_cases" name="closeCaseForm">
								<div class="modal-header">
									<h5 class="modal-title" id="confirmationModalTitle">Do you want to proceed ahead with closure of the
										case?</h5>
									<button type="button" class="close" data-dismiss="modal" aria-label="Close">
										<span aria-hidden="true">&times;</span>
									</button>
								...
								<div class="modal-body">
									...
									<p><i class="fa fa-check" aria-hidden="true"></i> Applicable Tax has been levied as per the provisions
										of GST acts/rules 2017</p>
									...
									<p><i class="fa fa-check" aria-hidden="true"></i> Applicable Penalty has been levied as per the
										provisions of GST acts/rules 2017</p>
									<p><input type="checkbox" id="mycheckbox" name="checkbox"> I hereby declare that above information is
										true and correct to the best of my knowledge</p>
								</div>
				...
				<div class="modal fade" id="updateCaseidModal" tabindex="-1" role="dialog" aria-labelledby="updateCaseidModal"
					aria-hidden="true">
					<div class="modal-dialog modal-dialog-centered" role="document">
						<div class="modal-content">
							<div class="modal-header">
							...
							<form method="POST" name="updatecaseidForm" id="updatecaseidForm" action="update_rejected_caseid"
									...
									<div class="row">
										<div class="col-md-12">
											<div class="form-group">
												...
												<input class="form-control" id="newcaseid" maxlength="15" pattern="[A-Za-z0-9]{15}"
													name="caseid" title="Please enter 15-digits alphanumeric Case Id" required />
											</div>
										</div>
									</div>
									...
									<div class="btn-group btn-group-vertical" role="group" data-toggle="buttons" style="width: 100%;">
										<c:forEach items="${listRemarks}" var="item">
											<label class="btn btn-outline-custom">
												<input type="radio" name="remarks" id="${item.id}" onclick="showHideOtherRemark()"
													value="${item.id}" required> ${item.name}
											</label>
										</c:forEach>
									</div>
									<textarea class="form-control" id="remarksId" name="otherRemarks" style="display: none"
										placeholder="Remarks"></textarea>

									<div class="form-group" style="margin-bottom: 0rem;">
										<label for="message-text" class="col-form-label">Upload File<span style="color: red;">
												*</span></label>
									</div>
					...
					function view(gst, date, period) {
						$.ajax({
							...
							success: function (result) {
								...
								if (result == 'true') {
									$("#updateSummaryViewBody").load('/fo/rejected_view_case/id?gst=' + gst + '&date=' + date + '&period=' + period,
							...
							$.ajax({
								...
								success: function (result) {
									...
									if (result == 'true') {
										...
										$("#updateSummaryViewBody").load('//fo/rejected_view_case', function (response, status, xhr) {
							...
							$.ajax({
								...
								success: function (result) {
									...
									if (result == 'true') {
										...
										$("#loader").show();
										setTimeout(function () {
											$("#dataListDiv").load('/fo/rejected_update_summary_data_list?id=' + categoryId + '&parameterId=' + selectedValue, function (response, status, xhr) {
												$("#loader").hide();
												if (status == 'success') {
													console.log("success");
												} else {
