...
function validateForm(){
  ...
  if(caseAssignedTo != '' & remarkOptions > 0){
    if(remarkOptions == 4){
      if(otherRemarks != ''){ return true; } else { alert("Please fill require information"); return false; }
    } else {
      return true;
    }
  } else {
...
function approve(){
  ...
  hidden_input1.type = 'hidden'; hidden_input1.name = 'gstIn'; hidden_input1.value = gstIn;

  const hidden_input2 = document.createElement('input');
  hidden_input2.type = 'hidden'; hidden_input2.name = 'caseReportingDate'; hidden_input2.value = caseReportingDate;

  const hidden_input3 = document.createElement('input');
  hidden_input3.type = 'hidden'; hidden_input3.name = 'period'; hidden_input3.value = period;

  const hidden_input4 = document.createElement('input');
  hidden_input4.type = 'hidden'; hidden_input4.name = 'assignedTo'; hidden_input4.value = assignedTo;

  const hidden_input5 = document.createElement('input');
  hidden_input5.type = 'hidden'; hidden_input5.name = '${_csrf.parameterName}'; hidden_input5.value = '${_csrf.token}';
  ...
  $.confirm({
    ...
    buttons : {
      submit : function() { hidden_form.submit(); },
...
function reject(){
  var x = document.getElementById("rejectRemarkDiv");
  if(x.style.display === "none"){ x.style.display = "block"; return; }
  ...
  hidden_input6.type = 'hidden'; hidden_input6.name = 'rejectRemark'; hidden_input6.value = rejectRemark;
...
<div class="modal fade bd-example-modal-lg" id="transferModal" tabindex="-1" aria-labelledby="transferModalTitle" aria-hidden="true">
    ...
    <div class="modal-content">
      ...
      <form method="POST" name="TransferForm" action="">
        <div class="modal-body">
          ...
          <!-- Was data-toggle="buttons" in BS4; harmless here, left as structural container -->
          <div class="btn-group btn-group-vertical" role="group" data-bs-toggle="buttons" style="width:100%;"></div>
