  ...
  <table id="dataListTable" class="table table-bordered table-striped table-responsive">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>
          <div><button type="button" style="margin-bottom:5px;" class="btn btn-primary viewbtn" onclick="view('${item.GSTIN_ID}' , '${item.caseReportingDate_ID}' , '${item.period_ID}')">Update</button></div>
          <c:choose>
            <c:when test="${item.actionStatusName eq 'Concluded'}">
              <div><button type="button" class="btn btn-primary" onclick="closeCase('${item.GSTIN_ID}' , '${item.caseReportingDate_ID}' , '${item.period_ID}')">Close</button></div>
            </c:when>
            <c:otherwise>
              <div><button type="button" class="btn btn-primary" disabled onclick="closeCase('${item.GSTIN_ID}' , '${item.caseReportingDate_ID}' , '${item.period_ID}')">Close</button></div>
            </c:otherwise>
          </c:choose>
          <c:if test="${item.caseIdUpdate eq 'yes'}">
            <div><button type="button" style="margin-bottom:5px;margin-top:5px;" class="btn btn-primary" onclick="updateCaseId('${item.GSTIN_ID}' , '${item.caseReportingDate_ID}' , '${item.period_ID}' , '${item.caseId}')">Update Case ID</button></div>
          </c:if>
        </td>
      </tr>
      </c:forEach>
...
<div class="modal fade" id="updateCaseidModal" tabindex="-1" aria-labelledby="updateCaseidModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <form method="POST" name="updatecaseidForm" id="updatecaseidForm" action="update_caseid" enctype="multipart/form-data">
        <div class="modal-body">
          ...
          <div class="form-group" style="margin-bottom: 0rem;">
            <label for="message-text" class="col-form-label">Upload File<span style="color: red;"> *</span>
              <span>(Only pdf files of size upto 10mb)</span>
            </label>
          </div>
...
<!-- JS (order matters): jQuery -> Bootstrap 5 -> AdminLTE 4 -> plugins -->
...
<!-- jQuery ↔ Bootstrap 5 Modal bridge so existing $('#id').modal('show') keeps working -->
<script>
  (function ($) {
...
// Disable back/forward cache & refresh (kept logic)
...
function view(gst, date, period){
  $.ajax({
    ...
    success: function(result){
      ...
      if(result=='true'){
        $("#updateSummaryViewBody").load('/fo/scrutiny_update_pop_up_view/id?gst='+gst+'&date='+date+'&period='+period, function(response, status, xhr){
          if(status == 'success'){
...
$(function () {
  ...
  $("#okayBtn").on("click", function(){ console.log("save"); $("#closeCaseModal").modal('hide'); });
  $("#closeBtn").on("click", function(){ console.log("failed"); $("#closeCaseModal").modal('hide'); });
});
</script>
      ...
      $.ajax({url: '/checkLoginStatus', method: 'get', async: false,
        success: function(result){
          ...
          if(result=='true'){
            ...
            $("#loader").show();
            setTimeout(function() {
              $("#dataListDiv").load('/fo/get_scrutiny_data_list?id=' + categoryId + '&parameterId=' + selectedValue, function(response, status, xhr) {
                $("#loader").hide();
